// src/components/dashboard/command-table.tsx
'use client';
...
const statusConfig: Record<CommandStatus, { label: string; className: string }> = {
  ...
  executing: { label: 'Executing', className: 'bg-blue-500/15 text-blue-400' },
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-muted text-xs uppercase tracking-wider border-b border-muted/10">
              ...
              <th className="px-4 py-3 text-right font-medium">Target L</th>
              <th className="px-4 py-3 text-right font-medium">Target R</th>
          ...
          <tbody>
            {commands.length === 0 ? (
              ...
              commands.map((cmd) => {
                const cfg = statusConfig[cmd.status] ?? statusConfig.failed;
                return (
                  <tr
                    key={cmd.id}
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 text-muted text-xs capitalize">{cmd.source}</td>
                    <td className="px-4 py-2.5 text-right text-text tabular-nums">{fmt(cmd.target_lux_l)}</td>
                    <td className="px-4 py-2.5 text-right text-text tabular-nums">{fmt(cmd.target_lux_r)}</td>
